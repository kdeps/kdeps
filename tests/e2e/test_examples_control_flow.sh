...
if [ "$SERVER_READY" = false ]; then
    fail_server_startup "Control Flow - Server startup (server did not start)" "$SERVER_LOG"
...
if command -v curl &> /dev/null; then
    ...
    if [ "$STATUS" = "200" ] || [ "$STATUS" = "500" ]; then
        ...
        test_failed "Control Flow - GET $ENDPOINT (status $STATUS)"
    fi
    ...
    if [ "$STATUS2" = "200" ] || [ "$STATUS2" = "500" ]; then
        ...
        test_failed "Control Flow - POST $ENDPOINT (status $STATUS2)"
    fi
fi
