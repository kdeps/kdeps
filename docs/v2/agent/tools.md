...
- **A recovered tool call is praised, not just a failed one flagged.** A model that fails a tool call and then succeeds on retry gets a `[GOOD]` acknowledgment in that same tool result - the positive counterpart to `[TOOL FAILED]`. It lands in conversation history, not just the terminal, so a model that "forgets how to call a tool" mid-session carries the correction into later tasks in the same session, not just the round it just finished. Never fires on an ordinary success with no prior failure - only a real recovery. The mandatory handshake's own tool result carries the same kind of line on a correct call.
