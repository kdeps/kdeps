...
func resolveConfig(
...
) (*domain.SearchLocalConfig, error) {
	...
	if resolved.Glob, err = evaluateStringOrLiteral(evaluator, ctx, config.Glob); err != nil {
		return nil, fmt.Errorf("failed to evaluate glob: %w", err)
	}
	if resolved.IndexDBPath, err = evaluateStringOrLiteral(evaluator, ctx, config.IndexDBPath); err != nil {
		return nil, fmt.Errorf("failed to evaluate indexDBPath: %w", err)
	}
	return &resolved, nil
}
