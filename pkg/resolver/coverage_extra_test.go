...
func TestTrivialCoverageBump(t *testing.T) {
	if out := formatDuration(3 * time.Second); out == "" {
	...
	got := convertToolParamsToString(params, "arg", "tool", logging.NewTestLogger())
	if got == "" {
		t.Fatalf("convertToolParamsToString returned empty string")
	}

	// exercise formatResponseData with simple input
	if out := formatResponseData(&apiresp.APIServerResponseBlock{Data: []any{"x"}}); out == "" {
	...
	_ = summarizeMessageHistory(msgs)
}
