...
All validation, filtering, and control-flow for a resource lives in a single `validations:` block. This replaces the former separate fields (`restrictToHttpMethods`, `restrictToRoutes`, `allowedHeaders`, `allowedParams`, `skipCondition`, `preflightCheck`, `validation`).
