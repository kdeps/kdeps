...
as any other provider. See [LLM Provider Reference](/llm-server/providers) for the other backends.
...
The provisioned Copilot agent is told that kdeps is a parser and an interpreter: it parses one matched `<invoke>` block out of the reply at runtime and interprets it. Not the model's code interpreter. Because tool calls are recovered from `<invoke>` blocks in free text, a written
...
- [LLM provider reference](/llm-server/providers) - all other backends
