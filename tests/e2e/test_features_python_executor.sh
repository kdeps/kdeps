...
if [ "$SERVER_READY" = false ]; then
    fail_server_startup "Python Executor - Server startup" "$SERVER_LOG"
...
if command -v curl &> /dev/null; then
    ...
    if [ "$STATUS_CODE" = "200" ]; then
        ...
        test_failed "Python Executor - POST endpoint (500 - Python may not be available or script error)"
    else
