...
| `graphBoost` | bool | no | `false` | Re-rank results using the same [kartographer](https://github.com/kdeps/kartographer) reference/topic graph `codeIntelligence`'s [`indexFolder`/`graphAll`](/resources/code-intelligence/graph) builds - requires `index: true` |
...
- [Graphing an indexed folder](/resources/code-intelligence/graph) - the same kartographer graph `graphBoost` reuses
