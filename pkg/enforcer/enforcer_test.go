...
func itHaveAAmendsURLLineOnTopOfTheFile(arg1 string) error {
	...
	if arg1 != "kdeps.com" {
		...
		if strings.Contains(doc, "Workflow.pkl") {
			doc = fmt.Sprintf(`amends "package://%s/core@0.3.3#/Workflow.pkl"
%s`, arg1, workflowValues)
		} else if strings.Contains(doc, "Resource.pkl") {
			doc = fmt.Sprintf(`amends "package://%s/core@0.3.3#/Resource.pkl"
%s`, arg1, resourceValues)
		} else {
			// Default to config file
			doc = fmt.Sprintf(`amends "package://%s/core@0.3.3#/Kdeps.pkl"
%s`, arg1, configValues)
		}
	} else {
		...
		if strings.Contains(doc, "Workflow.pkl") {
			doc = fmt.Sprintf(`amends "package://schema.kdeps.com/core@0.3.3#/Workflow.pkl"
%s`, workflowValues)
		} else if strings.Contains(doc, "Resource.pkl") {
			doc = fmt.Sprintf(`amends "package://schema.kdeps.com/core@0.3.3#/Resource.pkl"
%s`, resourceValues)
		} else {
			// Default to config file
			doc = fmt.Sprintf(`amends "package://schema.kdeps.com/core@0.3.3#/Kdeps.pkl"
%s`, configValues)
		}
	}
