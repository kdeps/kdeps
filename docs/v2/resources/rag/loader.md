...
`documents` is directly usable as the `documents:` input to a `vectorStore:` `add_documents` operation - each `content`/`metadata` pair maps onto a [`VectorStoreDocument`](/resources/rag/vector-store).
...
- [Vector store resource](/resources/rag/vector-store) - index and search the loaded documents
- [Embedding resource](/resources/rag/embedding) - lightweight keyword-based alternative for local, API-key-free RAG
