...
func TestNewEnvironment_DockerAutoDetection_SkipsConfigLookup(t *testing.T) {
	fs := afero.NewMemMapFs()
	// Create a config file in home directory
	_ = fs.MkdirAll("/home", 0o755)
	_ = afero.WriteFile(fs, "/home/.kdeps.pkl", []byte("test"), 0o644)
	// Create .dockerenv file for auto-detection
	_ = afero.WriteFile(fs, "/.dockerenv", []byte("x"), 0o644)

	// Set required Docker env vars for auto-detection
	t.Setenv("SCHEMA_VERSION", schema.SchemaVersion(context.TODO()))
	...
	env, err := NewEnvironment(fs, nil)
	assert.NoError(t, err)
	assert.Equal(t, "1", env.DockerMode, "DockerMode should be auto-detected")
	// Config file should NOT be found even though it exists, due to auto-detection
	assert.Empty(t, env.KdepsConfig, "KdepsConfig should be empty when Docker is auto-detected")
}
