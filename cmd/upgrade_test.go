...
AgentID = "testResource"`
  ...
  "version": "1.0.0"
	...
	t.Run("dry run upgrade", func(t *testing.T) {
		err := upgradeSchemaVersions(fs, testDir, "0.3.8", true, logger)
		require.NoError(t, err)
		...
		assert.NotContains(t, string(content), "0.3.8")
	})

	t.Run("actual upgrade", func(t *testing.T) {
		err := upgradeSchemaVersions(fs, testDir, "0.3.8", false, logger)
		require.NoError(t, err)

		// Check workflow.pkl was updated
		content, err := afero.ReadFile(fs, filepath.Join(testDir, "workflow.pkl"))
		require.NoError(t, err)
		assert.Contains(t, string(content), "0.3.8")
...
Version = "1.0.0"`
	...
	cmd := UpgradeCommand(fs, ctx, "/tmp", logger)
	cmd.SetArgs([]string{"--version", "0.3.8", testDir})
	...
	require.NoError(t, err)
	assert.Contains(t, string(updatedContent), "0.3.8")
...
AgentID = "test"`
	...
	newRef := matches[0][1] + "0.3.8" + matches[0][3]

	expected := `amends "package://schema.kdeps.com/core@0.3.8#/Workflow.pkl"
AgentID = "test"`
...
AgentID = "test"`
	...
	result, changed, err := upgradeSchemaVersionInContent(content, "0.3.8", logger)
	...
	require.Contains(t, result, "0.3.8", "Result should contain new version")
