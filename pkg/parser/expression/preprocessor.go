...
const (
	// minSubmatchCount is the minimum number of submatches required for a valid function call pattern.
	minSubmatchCount = 3
	// minTwoArgSubmatchCount is the minimum number of submatches for a two-argument function call.
	minTwoArgSubmatchCount = 4
)
...
func preprocessTemplate(template string) string {
	...
	result := simplePattern.ReplaceAllStringFunc(template, func(match string) string {
		...
		if len(submatch) >= minSubmatchCount {
			funcName := submatch[1]
			arg := submatch[2]
	...
	result = twoArgPattern.ReplaceAllStringFunc(result, func(match string) string {
		...
		if len(submatch) >= minTwoArgSubmatchCount {
			funcName := submatch[1]
			arg1 := submatch[2]
			arg2 := submatch[3]
