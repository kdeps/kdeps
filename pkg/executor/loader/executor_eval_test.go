...
func TestResolveConfig_InterpolatesFields(t *testing.T) {
	...
	cfg := &domain.LoaderConfig{
		Source:  "{{ get('a') }}{{ get('b') }}",
		Columns: []string{"{{ get('a') }}{{ get('b') }}"},
	}

	resolved, err := resolveConfig(ctx, cfg)
	require.NoError(t, err)
	require.Equal(t, "foobar", resolved.Source)
	require.Equal(t, []string{"foobar"}, resolved.Columns)
}

func TestResolveConfig_NilCtx_LeavesLiteral(t *testing.T) {
	cfg := &domain.LoaderConfig{Source: "{{ get('a') }}"}
	resolved, err := resolveConfig(nil, cfg)
	require.NoError(t, err)
	require.Equal(t, "{{ get('a') }}", resolved.Source)
}
