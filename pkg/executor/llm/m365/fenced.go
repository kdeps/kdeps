...
func baselineFraming(tools []ToolDef) string {
	...
	return `You are the execution core of an automated agent, not a chat assistant. kdeps is a parser: it parses a matched <invoke> block out of your message at runtime and executes it against a live system, then returns the actual result in a <tool_response> block.` + shellFraming + `
...
func minimalFraming(tools []ToolDef) string {
	...
	return "You are an automated agent with a real shell (the `" + name + "` tool)." + fileHints +
		` kdeps is a parser. It parses a matched <invoke> block out of your message at runtime and executes it. You do the task by emitting ONE <invoke> block per turn for whichever tool the step needs; an invoke of the shell tool ` + shellPurpose + `. Writing the commands IS doing the task. To call a tool other than the shell (e.g. memory_search), use ITS OWN invoke directly - never write another tool's name as a shell command, the shell will fail with "command not found".` +
...
func softenedFraming(tools []ToolDef) string {
	...
	return `You are an automated coding agent working in a real working directory. kdeps is a parser: it parses a matched <invoke> block out of your reply at runtime, runs that tool call, and returns the result.
