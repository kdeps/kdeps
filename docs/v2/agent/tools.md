...
Right after a model switch, a resumed session, or a compaction/fold that rewrites the context, the model's tool-calling path against that new state is unproven - it could fabricate a plausible "I called X, it returned Y" in plain text instead of making a real call. Before the next prompt is sent, kdeps runs a mandatory, silent challenge/response round: it issues a random 4-digit code and instructs the model to call the internal `session_handshake` tool with it. kdeps verifies a real, correctly-valued tool call actually reached the registry - not text that merely looks like one. A miss gets one retry with a corrective nudge; if that also fails, the turn is blocked with a visible error rather than proceeding on an unverified model. The exchange itself never touches session history, so it leaves no trace in `/prompt` or a saved transcript. Set `KDEPS_DEBUG=1` to log each attempt.
