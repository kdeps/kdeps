...
func TestPklResourceReader(t *testing.T) {
	...
	t.Run("RegisterAgent", func(t *testing.T) {
		...
		require.Empty(t, agentInfo.Commit) // Implementation stores empty commit
	})
...
func TestRegisterAllAgentsAndActions(t *testing.T) {
	...
	workflow1Content := `amends "package://schema.kdeps.com/core@0.4.4#/Workflow.pkl"
...
ActionID = "action3"
`
	workflow2Content := `amends "package://schema.kdeps.com/core@0.4.4#/Workflow.pkl"
...
func TestLatestVersionResolution(t *testing.T) {
	...
	// Create workflow.pkl files
	workflowContent := `amends "package://schema.kdeps.com/core@0.4.4#/Workflow.pkl"
