...
func TestClassifyModelArg(t *testing.T) {
	cases := []struct {
		...
		{"https://host/models/Qwen2.5-7B-Q4_K_M.gguf", kindGGUFURL, "Qwen2.5-7B-Q4_K_M"},
		{"https://host/rocket-3b.Q4_K_M.llamafile", kindLlamafileURL, "rocket-3b.Q4_K_M"},
		{"file:///models/local.gguf", kindGGUFURL, "local"},
		{"http://localhost:1234/v1", kindOpenAICompatURL, "localhost-1234"},
		{"https://api.example.com/openai/v1", kindOpenAICompatURL, "api.example.com-openai"},
		{"https://api.together.xyz/v1/", kindOpenAICompatURL, "api.together.xyz"},
		{"gpt-4o", kindNotURL, ""},
...
func TestHandleCustomModelURL_OpenAIEndpoint(t *testing.T) {
	...
	assert.Equal(t, "localhost-1234", loop.config.Model)
	assert.Equal(t, "openai", loop.config.Backend)
	assert.Equal(t, "http://localhost:1234/v1", loop.config.BaseURL)
	assert.True(t, repl.isModelName("localhost-1234"), "endpoint must be selectable")
	assert.Equal(t, "localhost-1234", savedAlias)
