...
Both [workflow mode](/workflow/) and [agent mode](/agent/).
...
`loader:`'s output documents map directly onto `vectorStore:`'s `documents:` field - see the [Loader Resource](/workflow/resources/loader#rag-pipeline-example) for the full load-then-index pipeline.

## See also

- [Loader resource](/workflow/resources/loader) - load files/URLs into documents before indexing
- [Embedding resource](/workflow/resources/embedding) - local SQLite-backed alternative, no external service required
