...
func (e *Engine) executeInlineResources(
	...
	for i, inline := range inlineResources {
		...
		case inline.Memory != nil:
			synthetic := &domain.Resource{
				Metadata: domain.ResourceMetadata{ActionID: fmt.Sprintf("_inline_%d", i)},
				Run:      domain.RunConfig{Memory: inline.Memory},
			}
			result, err = e.executeMemory(synthetic, ctx)
		case inline.Embedding != nil:
			synthetic := &domain.Resource{
				Metadata: domain.ResourceMetadata{ActionID: fmt.Sprintf("_inline_%d", i)},
				Run:      domain.RunConfig{Embedding: inline.Embedding},
			}
			result, err = e.executeEmbedding(synthetic, ctx)
		case inline.Scraper != nil:
			synthetic := &domain.Resource{
				Metadata: domain.ResourceMetadata{ActionID: fmt.Sprintf("_inline_%d", i)},
				Run:      domain.RunConfig{Scraper: inline.Scraper},
			}
			result, err = e.executeScraper(synthetic, ctx)
		case inline.Search != nil:
			synthetic := &domain.Resource{
				Metadata: domain.ResourceMetadata{ActionID: fmt.Sprintf("_inline_%d", i)},
				Run:      domain.RunConfig{Search: inline.Search},
			}
			result, err = e.executeSearch(synthetic, ctx)
		default:
...
func (e *Engine) executeLLM(resource *domain.Resource, ctx *ExecutionContext) (interface{}, error) {
	...
	if backendName == "" {
		backendName = backendNameOllama // Default
	}
...
// executeMemory executes a memory (key-value semantic store) resource.
// Uses SQLite with optional Ollama embeddings; falls back to keyword search.
//
//nolint:dupl,funlen // memory and embedding have structurally similar setup but different Python scripts
func (e *Engine) executeMemory(
	resource *domain.Resource,
	ctx *ExecutionContext,
) (interface{}, error) {
	kdeps_debug.Log("enter: executeMemory")
	cfg := resource.Run.Memory

	dbPath := cfg.DBPath
	if dbPath == "" {
		dbPath = "~/.kdeps/memory.db"
	}
	...
	script := fmt.Sprintf(`import sqlite3, os, json, math, sys

db_path = os.path.expanduser(%q)
...
def get_embedding(text):
    if not model or backend != 'ollama':
        return None
    try:
...
if operation == 'consolidate':
    ...
    conn.execute("INSERT INTO memory (content, category, embedding) VALUES (?,?,?)",
                 (content, category, json.dumps(emb) if emb else None))
    conn.commit()
    result = "consolidated"
    entries = []
    ...
    rows = conn.execute(
        "SELECT content, category, embedding FROM memory WHERE category=?",
        (category,)).fetchall()
    if not rows:
        result = "no entries found"
        entries = []
    else:
        q_emb = get_embedding(content)
        if q_emb and all(r[2] for r in rows):
            scored = [(cosine(q_emb, json.loads(r[2])), r[0]) for r in rows]
        else:
            kw = content.lower()
            scored = [(1.0 if kw in r[0].lower() else 0.0, r[0]) for r in rows]
            scored = [(s, t) for s, t in scored if s > 0] or [(0.0, r[0]) for r in rows]
        scored.sort(reverse=True)
        entries = [{"content": t, "score": round(s, 4)} for s, t in scored[:top_k]]
        result = "recalled"

elif operation == 'forget':
    conn.execute(
        "DELETE FROM memory WHERE category=? AND content LIKE ?",
        (category, "%%"+content+"%%"))
    conn.commit()
    result = "forgotten"
    entries = []
...
`, dbPath, operation, cfg.Content, category, cfg.Model, backend, baseURL, topK)

	executor := e.registry.GetPythonExecutor()
	if executor == nil {
		return nil, errors.New("python executor not available")
	}
	return executor.Execute(ctx, &domain.PythonConfig{Script: script})
}

// executeEmbedding executes a vector document store resource.
// Uses SQLite with optional Ollama embeddings; falls back to substring search.
//
//nolint:dupl,funlen // memory and embedding have structurally similar setup but different Python scripts
func (e *Engine) executeEmbedding(
	...
	cfg := resource.Run.Embedding

	dbPath := cfg.DBPath
	if dbPath == "" {
		dbPath = "~/.kdeps/embeddings.db"
	}
	operation := cfg.Operation
	if operation == "" {
		operation = "index"
	...
	script := fmt.Sprintf(`import sqlite3, os, json, math, uuid, sys
...
if operation in ('index', 'upsert'):
    emb = get_embedding(text)
    emb_json = json.dumps(emb) if emb else None
    if operation == 'upsert':
        row = conn.execute(
            "SELECT id FROM docs WHERE collection=? AND content=?",
            (collection, text)).fetchone()
        if row:
            conn.execute("UPDATE docs SET embedding=? WHERE id=?",
                         (emb_json, row[0]))
            conn.commit()
            doc_id = row[0]
        else:
            ...
            conn.execute(
                "INSERT INTO docs (id, collection, content, embedding) VALUES (?,?,?,?)",
                (doc_id, collection, text, emb_json))
            conn.commit()
    else:
        doc_id = str(uuid.uuid4())
        conn.execute(
            "INSERT OR REPLACE INTO docs (id, collection, content, embedding) VALUES (?,?,?,?)",
            (doc_id, collection, text, emb_json))
        conn.commit()
    result = {"id": doc_id, "operation": operation, "collection": collection, "success": True}

elif operation == 'search':
    rows = conn.execute(
        "SELECT id, content, embedding FROM docs WHERE collection=?",
        (collection,)).fetchall()
    q_emb = get_embedding(text) if text else None
    if q_emb and all(r[2] for r in rows):
        scored = [(cosine(q_emb, json.loads(r[2])), r[0], r[1]) for r in rows]
    else:
        kw = text.lower() if text else ''
        scored = [(0.99 if (not kw or kw in r[1].lower()) else 0.0,
                   r[0], r[1]) for r in rows]
        scored = [(s, i, c) for s, i, c in scored if s > 0]
    scored.sort(reverse=True)
    results = [{"id": i, "score": round(s, 4), "text": c}
    ...
    conn.execute(
        "DELETE FROM docs WHERE collection=? AND content=?",
        (collection, text))
    conn.commit()
    result = {"deleted": True, "operation": operation,
              "collection": collection, "success": True}

else:
    result = {"error": "unknown operation: " + operation, "success": False}

conn.close()
print(json.dumps(result))
`, dbPath, operation, cfg.Input, collection, cfg.Model, backend, baseURL, topK)
...
// executeScraper executes a file or URL content extraction resource.
func (e *Engine) executeScraper(
	resource *domain.Resource,
	ctx *ExecutionContext,
) (interface{}, error) {
	kdeps_debug.Log("enter: executeScraper")
	cfg := resource.Run.Scraper
	...
	script := fmt.Sprintf(`import json, os, sys
src_type = %q
source = %q
timeout = %d
selector = %q
...
if src_type == "url":
    try:
        ...
        req = urllib.request.Request(source, headers={"User-Agent": "kdeps/scraper"})
    ...
    with open(os.path.expanduser(source)) as f:
        raw = json.load(f)
    content = raw if isinstance(raw, (dict, list)) else str(raw)
elif src_type == "csv":
    with open(os.path.expanduser(source)) as f:
        content = f.read()
else:
...
print(json.dumps({"content": content, "type": src_type, "source": source, "success": True}))
`, srcType, cfg.Source, timeout, cfg.Selector)
...
// executeSearch executes a local file keyword/glob search resource.
func (e *Engine) executeSearch(
	resource *domain.Resource,
	ctx *ExecutionContext,
) (interface{}, error) {
	kdeps_debug.Log("enter: executeSearch")
	cfg := resource.Run.Search
	...
	script := fmt.Sprintf(`import os, json
...
`, searchPath, cfg.Query, cfg.Glob, limit)
