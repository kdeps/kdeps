...
The `searchLocal` executor walks a local directory and returns matching files by filename glob pattern and/or content keyword. See [Search resources](/workflow/resources/search) for how it relates to `searchWeb`.
...
- [Search resources](/workflow/resources/search) - overview and `searchWeb`
- [Graphing an indexed folder](/workflow/resources/code-graph) - the same kartographer graph `graphBoost` reuses
