...
func TestResolveConfig_InterpolatesFields(t *testing.T) {
	...
	require.NoError(t, ctx.Set("op", "hover"))

	cfg := &domain.CodeIntelligenceConfig{
		Operation: "{{ get('op') }}",
		Query:     "{{ get('a') }}{{ get('b') }}",
		Include:   []string{"{{ get('a') }}{{ get('b') }}"},
	}

	resolved, err := resolveConfig(ctx, cfg)
	require.NoError(t, err)
	require.Equal(t, domain.CodeIntelligenceOperation("hover"), resolved.Operation)
	require.Equal(t, "foobar", resolved.Query)
	require.Equal(t, []string{"foobar"}, resolved.Include)
}

func TestResolveConfig_NilCtx_LeavesLiteral(t *testing.T) {
	cfg := &domain.CodeIntelligenceConfig{Query: "{{ get('a') }}"}
