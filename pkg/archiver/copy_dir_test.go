...
func TestCopyFileMissingSource(t *testing.T) {
	fs := afero.NewMemMapFs()
	dst := "/dst.txt"
	if err := archiver.CopyFile(fs, context.Background(), "/no-such.txt", dst, logger); err == nil {
...
func TestMoveFolderSuccess(t *testing.T) {
	...
	// perform move
	if err := archiver.MoveFolder(fsys, "/src", "/dest"); err != nil {
...
func TestCopyFileHelpers(t *testing.T) {
	...
	if err := archiver.PerformCopy(fs, src2, dst); err != nil {
		t.Fatalf("performCopy error: %v", err)
	}

	if err := archiver.SetPermissions(fs, src2, dst); err != nil {
...
func TestParseActionIDEdgeCases(t *testing.T) {
	name, ver := archiver.ParseActionID("@other/action:2.1.0", "agent", "1.0.0")
	if name != "other" || ver != "2.1.0" {
		t.Fatalf("unexpected parse result %s %s", name, ver)
	}

	// Missing explicit name
	name2, ver2 := archiver.ParseActionID("myAction:0.3.0", "agent", "1.0.0")
	if name2 != "agent" || ver2 != "0.3.0" {
		t.Fatalf("unexpected default name parse")
	}

	// No version specified
	name3, ver3 := archiver.ParseActionID("@foo/bar", "agent", "1.2.3")
