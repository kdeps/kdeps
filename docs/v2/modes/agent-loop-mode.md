...
Goal enforcement checks that the loop keeps moving; it says nothing about whether
...
read a file, run a test, or search the web to verify a claim before ruling —
not just re-read the answer and guess. A judge settles its review by calling
