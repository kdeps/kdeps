...
# E2E test for complex-workflow example

set -uo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/common.sh"

echo "Testing Complex Workflow Example..."

WORKFLOW_PATH="$PROJECT_ROOT/examples/complex-workflow/workflow.yaml"
[ ! -f "$WORKFLOW_PATH" ] && { test_skipped "Complex Workflow (workflow not found)"; return 0; }
...
if "$KDEPS_BIN" validate "$WORKFLOW_PATH" &> /dev/null; then
    test_passed "Complex Workflow - Workflow validation"
else
    test_failed "Complex Workflow - Workflow validation" "Validation failed"
    return 0
fi
...
if [ "$SERVER_READY" = false ]; then
    ...
    test_skipped "Complex Workflow - Server startup (server did not start)"; return 0
fi
...
if command -v curl &> /dev/null; then
    ...
    if [ "$STATUS" = "200" ] || [ "$STATUS" = "500" ]; then
        ...
        test_skipped "Complex Workflow - POST $ENDPOINT (status $STATUS)"
    fi
fi
