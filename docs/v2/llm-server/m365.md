...
The provisioned Copilot agent is told that kdeps is a parser: it parses one matched `<invoke>` block out of the reply at runtime and executes it. Because tool calls are recovered from `<invoke>` blocks in free text, a written
