...
// package's actual resolver, covering a scalar field and the named-type
// Operation field (cast to/from string around the evaluator). This is also
// the live-confirmed original bug report: a "file:" resource's path: field
// was passed to the OS as the literal unresolved "{{ ... }}" string before
// this fix.
func TestResolveConfig_InterpolatesFields(t *testing.T) {
	ctx := newTestCtx(t)
	require.NoError(t, ctx.Set("dir", "/tmp"))
	require.NoError(t, ctx.Set("name", "/x.txt"))
	require.NoError(t, ctx.Set("op", "read"))

	cfg := &domain.FileResourceConfig{
		Operation: "{{ get('op') }}",
		Path:      "{{ get('dir') }}{{ get('name') }}",
	}

	resolved, err := resolveConfig(ctx, cfg)
	require.NoError(t, err)
	require.Equal(t, domain.FileResourceOperation("read"), resolved.Operation)
	require.Equal(t, "/tmp/x.txt", resolved.Path)
}

func TestResolveConfig_NilCtx_LeavesLiteral(t *testing.T) {
	cfg := &domain.FileResourceConfig{Path: "{{ get('dir') }}"}
	resolved, err := resolveConfig(nil, cfg)
	require.NoError(t, err)
	require.Equal(t, "{{ get('dir') }}", resolved.Path)
}
