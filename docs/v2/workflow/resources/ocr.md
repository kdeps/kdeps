...
Both [workflow mode](/workflow/) and [agent mode](/agent/). In agent mode, the same executor is available as the `ocr_image` built-in tool.
...
- [Transcribe resource](/workflow/resources/transcribe) - the equivalent offline, no-API-key pattern for audio (`whisper-cpp` backend)
