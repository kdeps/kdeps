...
# E2E test for inline-resources example
...
if "$KDEPS_BIN" validate "$WORKFLOW_PATH" &> /dev/null; then
    test_passed "Inline Resources - Workflow validation"
else
    test_failed "Inline Resources - Workflow validation" "Validation failed"
    return 0
fi
...
if [ "$SERVER_READY" = false ]; then
    fail_server_startup "Inline Resources - Server startup (server did not start)" "$SERVER_LOG"
...
if command -v curl &> /dev/null; then
    ...
    if [ "$STATUS" = "200" ] || [ "$STATUS" = "500" ]; then
        ...
        test_failed "Inline Resources - POST $ENDPOINT (status $STATUS)"
    fi
fi
