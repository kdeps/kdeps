...
// upgrade.Fresh/Detect/Perform, overridable in tests so /upgrade can be
// exercised without a real GitHub call, real install-method detection, or a
...
// cmdUpgrade implements "/upgrade": always performs a live check (this is an
// explicit user action, so a throttled cache would be misleading), then
// either prints package-manager instructions or -- for a standalone install
// -- confirms and downloads/verifies/replaces the running binary.
func (r *REPL) cmdUpgrade(_ []string) error {
	result, err := upgradeFreshFunc(r.ctx)
	if err != nil {
	...
	if instructions := upgrade.InstructionsFor(method); instructions != "" {
